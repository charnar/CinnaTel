...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `displayRooms` (IN `hotelid` BIGINT)  BEGIN
	SELECT DISTINCT roomtype.* FROM roomtype, room
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getHotelBranch` ()  BEGIN
	SELECT * FROM hotel;
END$$
...
INSERT INTO `booking` (`BookingID`, `HotelID`, `GuestID`, `PaymentID`, `DateFrom`, `DateTo`, `Adults`, `Children`, `ReserveDate`, `DiscountCode`, `ExtraInfo`) VALUES
...
(5, 'Crypto', '2021-11-02', 1, NULL),
(6, 'Kidney', '2021-11-07', 0, NULL),
(7, 'Credit Card', '2021-11-11', 1, NULL),
(8, 'Credit Card', '2021-11-01', 1, NULL),
...
--
DELIMITER $$
CREATE TRIGGER `checkBankSlip` BEFORE INSERT ON `payment` FOR EACH ROW BEGIN
	IF (NEW.BankSlip = '') THEN
    	SET NEW.BankSlip = NULL;
    END IF;
